...
let INPUTS: { [name: string]: InputFeature } = {
  x: { f: (x, y) => x, label: "Orders" },
  y: { f: (x, y) => y, label: "Discounts & Promotions" },
  xSquared: { f: (x, y) => x * x, label: "National Events" },
  ySquared: { f: (x, y) => y * y, label: "Demographic" },
  xTimesY: { f: (x, y) => x * y, label: "Historic Volume Data" },
  sinX: { f: (x, y) => Math.sin(x), label: "Stores Meta Information" },
  sinY: { f: (x, y) => Math.sin(y), label: "Weather" },
};
...
function drawNetwork(network: nn.Node[][]): void {
  ...
  let calloutThumb2 = d3.select(".callout.thumbnail2").style("display", "none");
  let calloutThumb3 = d3.select(".callout.thumbnail3").style("display", "none");
